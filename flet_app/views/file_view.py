...
class FileView(ft.UserControl):
        ...
        [
          ...
          ft.Row(
            [
              ...
              ft.OutlinedButton("フォルダ作成", icon=ft.icons.CREATE_NEW_FOLDER),
            ]
          ),
          ft.Divider(),
          # ファイル一覧
      ...
      content=ft.ListTile(
        ...
        trailing=ft.Row(
          [
            ...
            ft.IconButton(
              icon=ft.icons.DELETE, tooltip="削除", on_click=self.delete_file
            ),
          ],
          tight=True,
        ),
  ...
  def upload_file(self, e):
    ...
    self.page.show_snack_bar(
      ft.SnackBar(content=ft.Text("ファイルアップロード機能は実装予定です"))
    )

  def download_file(self, e):
    # TODO: ファイルダウンロード実装
    self.page.show_snack_bar(
      ft.SnackBar(content=ft.Text("ファイルをダウンロードしました"))
    )
  ...
  def open_file(self, e):
    # TODO: ファイル詳細表示実装
    self.page.show_snack_bar(
      ft.SnackBar(content=ft.Text("ファイル詳細画面は実装予定です"))
    )
